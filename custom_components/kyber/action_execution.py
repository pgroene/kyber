...
async def async_execute_actions(
    ...
    Returns a dict with keys: results, history_entry (optional), correction (optional).
    When ``approved`` is False, config-changing actions are blocked and the return dict
    will contain ``status: "approval_required"`` and ``blocked_actions`` instead.
    """
            ...
            try:
                ...
                if svc_entity_id and service in ("turn_on", "turn_off", "toggle"):
                    try:
                        ...
                        label_info = await async_infer_and_apply_label(hass, svc_entity_id, plan_summary)
                        if label_info:
    ...
    if _has_service_failures:
        try:
            ...
            correction = await async_try_correct_failures(
                hass, results, actions, plan_summary
            )
            if correction:
                _LOGGER.info(
    ...
    if applied_actions and not has_failures:
        try:
            ...
            history_entry = await astore.async_record(
                plan_summary or "Applied Kyber actions",
                applied_actions,
                entity_changes,
                user_id=user_id,
