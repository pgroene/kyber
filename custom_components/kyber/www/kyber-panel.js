...
// Styles
// ---------------------------------------------------------------------------
import { STYLES } from "./src/styles.js?v=217";
...
import { EditorMixin } from "./src/editor-mixin.js?v=215";
import { AIMixin } from "./src/ai-mixin.js?v=120";
import { PlanCardsMixin } from "./src/plan-cards-mixin.js?v=116";
...
class KyberPanel extends AIMixin(PlanCardsMixin(SlashMixin(EditorMixin(DebugMixin(KnowledgeMixin(SessionMixin(UtilsMixin(HTMLElement)))))))) {
  ...
  _bindEvents(shadow) {
    ...
    shadow.getElementById("btn-history-toggle").addEventListener("click", () => this._toggleActionHistory());
    shadow.getElementById("btn-history-refresh").addEventListener("click", () => this._loadActionHistory());
