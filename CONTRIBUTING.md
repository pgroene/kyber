...
Before tagging, verify all version references are in sync:

- [ ] `custom_components/kyber/manifest.json` — `"version"` matches the new tag
- [ ] `custom_components/kyber/www/manifest.json` — same version as above
- [ ] `CHANGELOG.md` — `[Unreleased]` header updated to the new version
- [ ] GitHub Release notes written (auto-generated by `scripts/release.py` or manually via `gh release create`)
